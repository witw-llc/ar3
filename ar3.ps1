...
foreach ($Candidate in $Candidates) {
    ...
    if ($Candidate.Override) {
        [Console]::Error.WriteLine("ar3: AR3_PYTHON=$($Candidate.Name) does not run; falling back to PATH")
    }
}
if (-not $PythonExe) {
...
if ($MyInvocation.ExpectingInput) {
    ...
    $input | & $PythonExe @PythonArgs $Ar3Path @args
} else {
