...
foreach ($Candidate in $Candidates) {
    ...
    if ($Candidate.Override) {
        [Console]::Error.WriteLine("r4t: AR3_PYTHON=$($Candidate.Name) does not run; falling back to PATH")
    }
}
if (-not $PythonExe) {
    [Console]::Error.WriteLine("r4t: no working python3, python, or py on PATH")
    exit 127
}
...
if ($MyInvocation.ExpectingInput) {
    ...
    $input | & $PythonExe @PythonArgs $R4tPath @args
} else {
    & $PythonExe @PythonArgs $R4tPath @args
}
exit $LASTEXITCODE
