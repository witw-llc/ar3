...
directories under `$XDG_DATA_HOME/ark/deps`. Every test here gets a private
...
import ar3
from ark import deps as ark_deps
...
class TestDepsRoot:
    def test_honors_xdg_data_home(self, tmp_path):
        assert ark_deps.deps_root() == tmp_path / "xdg-data" / "ark" / "deps"

    def test_defaults_to_home_dot_local_share_when_xdg_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("XDG_DATA_HOME", raising=False)
        monkeypatch.setenv("HOME", str(tmp_path / "home"))
        assert ark_deps.deps_root() == tmp_path / "home" / ".local" / "share" / "ark" / "deps"
...
class TestGroupDir:
    def test_layout_is_root_then_interpreter_then_group(self):
        assert ark_deps.group_dir("r4t") == (
            ark_deps.deps_root() / ark_deps.interpreter_key() / "r4t"
        )
...
class TestEnsureGroup:
    def test_missing_dir_returns_none(self):
        assert ark_deps.ensure_group("no-such-group-at-all") is None

    def test_satisfied_dir_returns_its_path(self, monkeypatch, tmp_path):
        req_dir = tmp_path / "requirements"
        req_dir.mkdir()
        (req_dir / "fakegroup.txt").write_text("fakepkg>=1.0\n", encoding="utf-8")
        monkeypatch.setattr(ark_deps, "REQUIREMENTS_DIR", req_dir)
        dir_ = ark_deps.group_dir("fakegroup")
        dir_.mkdir(parents=True)
        _write_dist(dir_, "fakepkg", "1.2.3")
        assert ark_deps.ensure_group("fakegroup") == dir_
    ...
    def test_unsatisfied_pin_returns_none(self, monkeypatch, tmp_path):
        ...
        dir_.mkdir(parents=True)
        _write_dist(dir_, "fakepkg", "1.2.3")
        assert ark_deps.ensure_group("fakegroup") is None
    ...
    def test_no_pin_file_trusts_an_existing_dir(self, tmp_path):
        ...
        dir_ = ark_deps.group_dir("no-pins-group")
        dir_.mkdir(parents=True)
        assert ark_deps.ensure_group("no-pins-group") == dir_
...
class TestInstallGroup:
    ...
    def test_prefers_uv_when_on_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(ark_deps.subprocess, "run", fake_run)
        ark_deps.install_group("fakegroup")
    ...
    def test_falls_back_to_pip_module_when_uv_absent(self, monkeypatch, tmp_path):
        self._fake_group(monkeypatch, tmp_path)
        monkeypatch.setattr(ark_deps.shutil, "which", lambda _name: None)
        seen = {}
    ...
    def test_success_swaps_the_tmp_dir_into_place_and_leaves_no_litter(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(ark_deps.subprocess, "run", fake_run)
        dest = ark_deps.install_group("fakegroup")
        assert dest == ark_deps.group_dir("fakegroup")
    ...
    def test_success_replaces_a_previous_install(self, monkeypatch, tmp_path):
        self._fake_group(monkeypatch, tmp_path)
        monkeypatch.setattr(ark_deps.shutil, "which", lambda _name: None)
        final_dir = ark_deps.group_dir("fakegroup")
...
class TestUseGroup:
    def test_missing_group_warns_and_returns_false(self, capsys):
        assert ark_deps.use_group("no-such-group-anywhere") is False
    ...
    def test_fetched_group_shadows_an_older_copy_on_site_packages(self, monkeypatch, tmp_path):
        ...
        site_packages.mkdir()
        (site_packages / "ark_deps_probe_mod.py").write_text(
            "SOURCE = 'stale-site-packages'\n", encoding="utf-8"
        )

        dir_ = ark_deps.group_dir("fakegroup")
        dir_.mkdir(parents=True)
        (dir_ / "ark_deps_probe_mod.py").write_text(
            "SOURCE = 'fresh-group-dir'\n", encoding="utf-8"
        )
        monkeypatch.setattr(ark_deps, "ensure_group", lambda _g: dir_)

        original = list(sys.path)
        sys.modules.pop("ark_deps_probe_mod", None)
        try:
            ...
            import ark_deps_probe_mod

            assert ark_deps_probe_mod.SOURCE == "fresh-group-dir"
        finally:
            sys.modules.pop("ark_deps_probe_mod", None)
            sys.path[:] = original
