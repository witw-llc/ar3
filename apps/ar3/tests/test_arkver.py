...
`arkver` lives at the repo root because all six CLIs answer `--version` from
...
def fake_tags(monkeypatch):
    ...
    def install(names, error=None):
        ...
        monkeypatch.setattr(arkver.urllib.request, "urlopen", fake_urlopen)
    return install


class TestSuiteVersion:
    def test_reads_the_repo_version_file(self):
        expected = (arkver.repo_root() / "VERSION").read_text().strip()
        assert arkver.suite_version() == expected

    def test_version_line_names_the_app_and_the_suite(self):
        line = arkver.version_line("a8s")
        assert line.startswith("a8s ")
        assert arkver.suite_version() in line
    ...
    def test_missing_version_file_is_not_a_crash(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(arkver, "repo_root", lambda: tmp_path)
        assert arkver.suite_version() == arkver.UNKNOWN
...
class TestUpdateNote:
    def test_names_the_newer_version_when_behind(self, monkeypatch):
        monkeypatch.setattr(arkver, "suite_version", lambda: "0.1.57")
        monkeypatch.setattr(arkver, "latest_public_version", lambda timeout_s=3.0: "0.1.58")
        note = arkver.update_note()
        assert "0.1.57" in note and "0.1.58 is available" in note

    def test_says_latest_when_current(self, monkeypatch):
        monkeypatch.setattr(arkver, "suite_version", lambda: "0.1.58")
        monkeypatch.setattr(arkver, "latest_public_version", lambda timeout_s=3.0: "0.1.58")
        assert arkver.update_note() == "0.1.58 (latest)"
    ...
    def test_a_working_copy_ahead_of_the_mirror_is_not_an_update(self, monkeypatch):
        ...
        monkeypatch.setattr(arkver, "suite_version", lambda: "0.1.58")
        monkeypatch.setattr(arkver, "latest_public_version", lambda timeout_s=3.0: "0.1.57")
        assert arkver.update_note() == "0.1.58 (latest)"

    def test_offline_says_so_without_claiming_to_be_current(self, monkeypatch):
        monkeypatch.setattr(arkver, "suite_version", lambda: "0.1.58")
        monkeypatch.setattr(arkver, "latest_public_version", lambda timeout_s=3.0: None)
        note = arkver.update_note()
