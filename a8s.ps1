...
foreach ($Candidate in $Candidates) {
    ...
    if ($Candidate.Override) {
        [Console]::Error.WriteLine("a8s: AR3_PYTHON=$($Candidate.Name) does not run; falling back to PATH")
    }
}
if (-not $PythonExe) {
    [Console]::Error.WriteLine("a8s: no working python3, python, or py on PATH")
    exit 127
}
...
if ($MyInvocation.ExpectingInput) {
    ...
    $input | & $PythonExe @PythonArgs $A8sPath @args
} else {
    & $PythonExe @PythonArgs $A8sPath @args
}
exit $LASTEXITCODE
