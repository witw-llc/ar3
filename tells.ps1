...
foreach ($Candidate in $Candidates) {
    ...
    if ($Candidate.Override) {
        [Console]::Error.WriteLine("tells: AR3_PYTHON=$($Candidate.Name) does not run; falling back to PATH")
    }
}
if (-not $PythonExe) {
    [Console]::Error.WriteLine("tells: no working python3, python, or py on PATH")
    exit 127
}
...
if ($MyInvocation.ExpectingInput) {
    ...
    $input | & $PythonExe @PythonArgs $A8sPath tells @args
} else {
    & $PythonExe @PythonArgs $A8sPath tells @args
}
exit $LASTEXITCODE
