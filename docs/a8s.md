...
| `a8s ls [-q]`                  | List every name this node can reach. Registered nodes first, running or not: NAME, STATUS (`running (pid N)` / `stopped` / `unresolved: $KEY`), DEFINITION, ROOT, and bound namespaces. `running` means the pid is alive and is the same process that claimed the node: its start time is stamped beside the pid, so a number the OS reused after a reboot reads `stopped` (Linux and macOS; Windows checks the pid alone). Then names only ever heard over a remote, with DEFINITION `remote` and the local time each last reached you. `-q` prints just names, both kinds. |
...
may own the binding), held for retry while a configured remote failed to
start, or trashed when none is configured. Removing an agent unbinds
...
| `a8s restart <name> [<name> ...] [--force]` | `stop` (wait until detached) then `start`, name by name. `--force` is passed to stop. A name not running just starts. The exit code is nonzero if any name failed. |
...
A node told to stop by a signal writes the signal and the parent it found into its log and into `RUN_STOP` (`a8s tx --event RUN_STOP`), e.g. `stop-signal SIGTERM (15) from ppid 1 (systemd)` for a host shutdown.

#### After a host restart

Nothing re-attaches a node when its machine boots. The shutdown SIGTERMs the node, which detaches; mail in its outbox, or parked for a backoff retry, stays on disk and publishes on the next start with no other step. To start the node at boot, have the machine run it — `a8s run` under a supervisor that owns the process, `a8s start` from a line that runs once:

- **cron:** `@reboot /path/to/ar3/a8s start <name>`
...
| `tell <name> [<msg>\|-]` (top-level shim, `tell` at the repo root) | Delegates to `a8s tell` (`apps/a8s/tell.py`). Outbox: `TELL_OUTBOX_DIR` if set, else a unique configured outbox matched from CWD when the a8s state root is readable (see [filedrop.md](a8s-filedrop.md)). Drops a JSON envelope. When the registry is reachable, `from` stamping applies; recipient validation applies only when the resolved outbox is a registered one (a staging outbox belongs to another router — see [a8s-tell.md](a8s-tell.md#who-validates-the-recipient)). Windows: `tell.cmd`, whose argv is **not** native-equivalent — `cmd.exe` truncates any argument at its first newline, so a multi-line body and any `FILE:` line with it are lost silently; pass those on stdin or `--file` (#230). PowerShell: `tell.ps1` forwards pipeline input in UTF-8 whatever the caller's `$OutputEncoding` is, so `$body | tell.ps1 <name> -` and a piped here-string reach the message whole, non-ASCII included. `AR3_PYTHON` points any launcher at a bundled interpreter ahead of PATH, and the `.cmd`/`.ps1` launchers run the child in UTF-8 mode unless the caller set `PYTHONUTF8` or `PYTHONIOENCODING` (see [ar3.md](ar3.md)). `tell` never blocks for an outcome — read what became of a send with `tells --sent`. Operator internals: [a8s-tell.md](a8s-tell.md). |
| `tells [-f] [--timeout SEC] [--live] [--body-max N] [--line-max N] [--glow [theme]] [--sent [--since D]]` (shim `tells` at the repo root) | Receive-side complement of `tell` (`apps/a8s/tells.py`). Same outbox resolution as `tell`; watches `.inbox` beside it. Default: wait with no time limit for the next burst, print it, exit 0; `--timeout SEC` bounds the wait. `-f` / `--timeout 0`: follow until Ctrl+C. On the plain (non-`--glow`, non-heading) path, a printed line over `--line-max` / `TELLS_LINE_MAX` bytes (default 400; `0` = no wrap; a positive value under **16** is refused, since the two-byte continuation indent plus one four-byte character could not fit inside it) is soft-wrapped at the last space under the limit, with a two-space indent on continuation lines. A break consumes exactly one space, so stripping the indent and rejoining the lines on a single space recovers the original body byte for byte — runs of spaces, leading and trailing whitespace included — unless a single word was too long for the limit and had to be hard-broken — Claude Code's Monitor notification clips each stdout line at 500 bytes, and a paragraph over that loses its tail with no sign anything was cut. Bodies over `--body-max` / `TELLS_BODY_MAX` (default 16000; `0` = unlimited), and any message a line of which needed wrapping, print a `tells --recover <token>` command in the header, before the body, so a host that clips mid-message still shows where the whole thing is — the token is an encoded path, so no shell rewrites it. `--glow` / headings share convo's markdown formatting, unwrapped. A message delivered more than 10 minutes after the sender queued it prints with a `[late 32h]` prefix, so replay is distinguishable from live traffic. In follow mode, whatever is already in the inbox at arm time prints first under a `--- N message(s) already in the inbox when tells started at HH:MM; live from here ---` boundary, each carrying a `[backlog]` prefix (`[backlog] [late 32h]` when both apply) — a re-armed monitor never reads old mail as live. `--live` skips that backlog and prints only what arrives after the arm; it requires `-f`/`--timeout 0`. `--sent [--since 2h]` lists this seat's own outbound ULIDs with their latest delivery state, age and recipient, read from the receipt files in its own outbox — no registry, no machine-wide log. Non-destructive. Prefer over `convo -f` for filedrop inbound-only loops. |
| `a8s logs <name>... [--tail N] [-f]`                                        | Read per-agent log files; one agent in append order, multiple merge by ISO timestamp. `-f` follows.                                                                                                                                                                                                                       |
| `a8s convo <name> [--limit N] [-f] [--from NAME] [--since CURSOR] [--json] [--glow [theme]]` | Markdown history of messages to or from an agent. Without `--since`, the newest `--limit` messages (default 10); this controls display only. `-f` follows sequence-numbered rows in `conversations.sqlite3` (shows outbound too — use `tells -f` for filedrop inbound-only). `--from NAME` (repeatable, case-insensitive) keeps only that sender's messages, in the backlog and under `-f`; the limit counts matches, not rows scanned. `--since CURSOR` keeps only rows after CURSOR — a message ulid (rows with a greater `seq`, delivery order), an ISO timestamp or bare date (rows after the last-inserted row dated at or before it, strictly before it for a bare date, so the day is whole and its midnight row is in; a bare date or a timestamp without an offset is read in the machine's local time, and dates are compared as instants, not as text), or a duration like `2h`/`30m`/`3d` (rows dated within that span of now). **`--since` on its own returns every row after the cursor, oldest first — a drain.** Add `--limit N` for the oldest N of them and continue from the newest `ulid` you were handed, and no message between the cursor and the window is skipped. Composes with `--from`; finding nothing new prints nothing and exits 0 (#276). `--json` prints one newline-delimited object per row (`ulid`, `seq`, `from`, `to`, `utc`, `content`, `files`, `files_unavailable`) instead of markdown; `files_unavailable` holds one `{filename, error, detail}` object per attachment the transfer could not deliver — empty when every file arrived — so a consumer never reads a lost attachment as a delivered one. `a8s update` retains `convo_max_rows` rows (default 50000). Exit 0 with nothing printed means no messages; a store that is missing, unreadable, or not a conversation store at all is named on stderr with exit 1, and reading opens read-only — it never creates or initializes the file. `a8s transactions` and `a8s trace` draw the same line over `transactions.sqlite3`. |
...
| `a8s remote <name> <broker-url> <topic> [--<opt> <value> ...]` | Register or overwrite a broker remote. Broker URL is `mqtt://host[:1883]` or `mqtts://host[:8883]`. Persistent session + QoS 1 are wired automatically so an offline cluster catches up on reconnect. Non-secret opts land in `network.json`; `--pass` / `--password` go to `secrets.json` (0600) in the same command — `--pass` is optional. Unknown options are rejected by the transport at load time so typos fail loud. Options take either spelling — `--user alice` or `--user=alice`. |
| `a8s remote <name> <folder> [--<opt> <value> ...]`             | Register or overwrite a **folder** remote — envelopes travel through a directory a sync client already watches, with no broker, port or account. One place instead of two is what tells the two forms apart, so **quote a path with a space in it** — `a8s remote box "G:/My Drive/A8S"` — or the shell hands a8s three words and the broker form answers. `--poll-seconds` (15, floored at 1), `--prefix` (none — the typed path is the base), `--retain-days` (3 days; `0` keeps forever). The same command registers that folder as a `sync_folder` storage service under the same name, so a message's attachments travel with it, and overwriting the remote with a new path moves that service to the new path too. Registration stamps a `joined` ULID into the spec — envelopes named below it are the backlog this machine is not owed, whenever they turn up. It is written, never typed, and `a8s remote <name>` shows it as the ULID and the moment it names. Re-registering the same folder *and* the same `--prefix` keeps that cutoff, since only options changed; a different folder or a different prefix is a different mailbox and stamps a new one. |
| `a8s remote <name> s3://<bucket>/<prefix> [--<opt> <value> ...]` | Register or overwrite an **s3** remote — envelopes travel through a bucket, which is the answer for a machine that can reach HTTPS on 443 and nothing else. One place, like the folder form; the scheme picks between them. Every machine that holds a name gets its own copy of that name's mail, and mail leaves the bucket when `--retain-days` runs out, not when somebody reads it. The prefix is required and must be one nothing else in the bucket writes under, because that retention reap deletes under it — `a8s remote` and `a8s storage` each refuse a prefix the other already uses. Set a bucket lifecycle rule on the prefix with the same window; it is the durable backstop. `--poll-seconds` (10, floored at 1), `--retain-days` (3 days; `0` keeps forever), `--region`, `--profile`, `--endpoint-url`, `--timeout-s` (60). Registration stamps a `joined` ULID into the spec as the folder form does, and keeps it when only options change. Registration installs `boto3` through the `a8s-s3` group; credentials come from the standard chain (env, shared config, SSO, instance and container roles) and a8s never reads, stores or logs one. Attachments need their own service under a different prefix — `a8s storage <name> s3://<bucket>/a8s-files`. |
| `a8s unremote <name>`                                          | Forget a remote. Running daemons keep using the prior config until restart. A folder or s3 remote's consumed-envelope ledger goes with it, and so does the `sync_folder` service `a8s remote` created for that folder — identified by the `paired` marker that registration wrote into it, so a same-name service you configured yourself stays put even when it points at the same folder.                                                                                                                                                                                                                                                                                                    |


Remotes are git-shaped: an explicit list of places to fan messages out to. a8s only crosses cluster boundaries on `tell` / `prompt` — everything else (`a8s logs`, `a8s ls`, `a8s ps`) reads local state and nothing else. `a8s ls` naming remote nodes is not an exception: it reports who has reached *you*, read out of this machine's own address book, and asks nothing of the far side. Every arrival updates one row per name (folded by case, spelled the way the newest arrival spelled it) in `transactions.sqlite3`, beside the event log but outside its `txlog_max_rows` retention, so a name last heard from weeks ago still lists: the book records arrival, not liveness. If you want cross-cluster log access, register an a8s connector that turns inbound tells into local `a8s logs` calls; a8s itself just enables the message + invocation path.

Each handler claims its own broker session: the MQTT client id is a hash of the machine, the handler's attached agent set, and the remote name, so several nodes share a host without evicting each other, and each one's queued messages survive a restart. `A8S_CLIENT_TAG` replaces the agent-set part, and a `client_id` option on the remote replaces the whole id. `a8s health` connects with a throwaway id and a clean session so a connectivity check never touches a node's queue, and reports `FAIL (not connected)` when the broker has not answered within the remote's `connect_timeout_s`.
...
`a8s remote hub s3://my-bucket/a8s-mail` is the third wire, and the one for a machine behind an egress proxy that allows `CONNECT` on 443 to a list of domains and refuses every other port. mqtt wants 1883 or 8883 and a folder wants a shared filesystem; a bucket wants one allowed domain on 443, and the machine pulls, so nothing has to reach it from outside. The layout is one object per envelope under a prefix per recipient — `<prefix>/<recipient>/<ULID>.json`, with `<prefix>/<recipient>/<ULID>/` reserved for that message's attachments — so a poll is one `list_objects_v2` against one mailbox with a `/` delimiter, no filtering, and ULIDs sort lexically so the listing already comes back in send order. At the default 10-second interval that is about 260k LIST requests a month, roughly USD 1.30, for each mailbox-and-daemon pair: every daemon on the node runs its own subscriber over the same registry, so three daemons holding four mailboxes between them poll twelve times over. `--poll-seconds` is the dial and the arithmetic is linear in both.

Every machine that holds a name gets a copy of its mail, as every MQTT subscriber does. A node polls `<prefix>/<name>/` for every agent, alias and namespace prefix in its registry — re-read on every poll so `a8s add` needs no restart — and a name may live on several machines at once. No reader deletes what it reads. Each machine keeps its own ledger of consumed ULIDs per remote under `~/.config/a8s/s3-remotes/`, a poll skips what the ledger names without fetching it, and the object stays in the bucket for the other machines. A publish goes into the bucket even when this node also holds the recipient: the routing pass delivers locally and records the ULID in `seen-ids` first, exactly as it does before an MQTT publish, and this node's own poll meets the object the way an MQTT client meets the broker's echo — the ring answers, and nothing is written twice. A node is not a process: every daemon on the machine runs its own subscriber, so several of them poll the same mailbox. Whichever wins `claim_message` delivers and stamps the ledger; the rest are told they did not, and leave it unrecorded so it is offered again. Delivery is at-least-once, and `claim_message` plus the seen-ids ring collapse the repeats. A machine that joins is owed the mail sent after it joined: registration stamps `joined`, and an envelope minted more than an hour below it is history, with the same clock allowance as the folder remote.

Mail leaves the bucket by time. `--retain-days` sweeps every object in a mailbox this node polls — consumed or not, well-formed or junk — once both the ULID mint time and the object's `LastModified` clear the window, the same two clocks the folder sweep requires, so a backoff retry that republishes an old envelope is delivered rather than swept; a key carrying no ULID has only the object clock to offer. `0` keeps everything. Set a bucket lifecycle rule on the prefix with the same window: it survives every node going away and covers a mailbox no node polls any more, and nothing in a8s does either. A poll reads one page of at most 1000 keys per mailbox and the next poll resumes where it stopped, so a poll is one LIST per mailbox however much the mailbox holds. A consumed envelope costs its share of that LIST and no GET, and a mailbox holding N retained keys shows new mail within ceil((N + 1) / 1000) polls — one poll up to 999 retained keys, two at 1000. `a8s health` exercises PUT, GET, LIST and DELETE against the configured prefix and removes its own probe — an IAM policy that grants three of the four is invisible until the missing verb is the one a message needs.
...
A kind that needs a heavy package (today, `s3` needs `boto3`) installs it the moment `a8s storage` — or `a8s remote`, for the s3 transport — registers that kind. There is no second command to remember. `ar3 deps <group>` is the explicit form of the same mechanism, for scripts and for `ar3 doctor`.
...
Redundancy is **any-of**. Every send attempts every service, so the good case is a copy in all of them, but a message publishes as soon as each file has landed somewhere. One unreachable service cannot hold the others hostage — which is the whole reason to configure a second one. Only services that accepted a file contribute a URL, so a failed service publishes nothing rather than a link that cannot resolve. The floor is one, not zero: a file no service accepted keeps the retry, because an envelope whose attachment nobody can fetch is a message lost quietly.
...
A sender that could not upload at all says so the same way, by publishing an entry with `error` and no `storage`. The receiver keeps it and delivers it as metadata, carrying the sender's reason rather than restating it as a local download failure, and does not wait on it — there is no URL to retry, so waiting would hold the message for the full window and then deliver what was already available. Only a filename-only entry with neither `storage` nor `error` is dropped, which is the legacy shape that rule exists for.

Attachment URLs must be **https**. A peer chooses the URL your node fetches, and presigned links carry their own authorization in the query string, so plaintext is refused. A download follows at most three redirects and applies the same rule at every hop — object stores routinely redirect a share URL to the host that holds the bytes, so refusing outright would break those links. `storage_allow_http=1` relaxes the scheme rule for a store on your own network with no certificate. A connection cut mid-download is a failure, not a partial delivery: the byte count is checked against the response's `Content-Length` before the file is kept, so a short read raises the same as an unreachable host and goes through the normal retry instead of landing as a truncated file.

`sync_folder` is the one kind that publishes nothing at all. Give it a folder your sync client already watches — `a8s storage onedrive "~/OneDrive - Contoso/A8S"` — and give a second machine the same folder. a8s copies an attachment in on send and looks for it on receive; neither side knows or cares how the bytes travelled. Attachments are keyed by message ULID, so one message's files stay together in `<folder>/<ULID>/`. The marker that rides in the envelope names neither the service nor the path, which means the two machines need not even agree on what to call it, and configuring two folders makes them race: whichever syncs first delivers.

The hazard is not the network, it is a half-arrived file. A sync client publishes a name before the bytes behind it are complete, and OneDrive's Files On-Demand shows a placeholder that only materializes when something reads it. So a file is written under a `.part` name and renamed, and a `manifest.json` records the size the receiver must see before it will accept the copy. A file that does not match yet simply is not there yet — the receiver's normal wait keeps looking. The folder is a wire, not an archive: `--retain-days` sweeps bundles older than the window by both their ULID mint time and their mtime, 3 days by default, and `--retain-days 0` keeps them forever. Delivered attachments are already in the recipient's own `.files/`, so what stays behind in the shared folder is only ever a spare copy.

The same folder carries the messages too. `a8s remote box "~/Dropbox/A8S"` writes one file per envelope — `<folder>/<ULID>.json`, the same bytes MQTT carries — beside the `<folder>/<ULID>/` bundles the storage service writes, so a machine pair with a shared folder needs no broker at all. `a8s remote` registers both in one command for exactly that reason. Nothing deletes an envelope on receive: every machine sharing the folder wants to read it, and each keeps its own ledger of consumed ULIDs under `~/.config/a8s/folder-remotes/`, never in the folder. That ledger keeps every retained envelope: it compacts against what is still in the folder rather than dropping its oldest lines, because a forgotten ULID is a duplicate delivery at the next restart. A machine joining an old folder is not owed its backlog, and what enforces that is the `joined` ULID stamped at registration, not a reading of the folder — the folder may be unmounted at registration and download a month of history an hour later, and every envelope named below the cutoff stays history. A ULID is ordered by the clock that minted it, so the cutoff carries an hour of skew allowance: a peer whose clock runs behind yours still reaches you, and the price is that joining a busy folder can deliver up to an hour of its recent traffic as current mail. Keep the machines' clocks within that hour. A clock more than an hour *ahead* at registration stamps a cutoff no peer can reach and the node stays deaf until you clear it — `a8s unremote <name>` and register again is how a machine re-joins at a new cutoff, and the poll says how many envelopes it is discarding as backlog so the log shows you which case you are in.

`--retain-days` is the only thing that removes an envelope — 3 days by default, `0` disables it — and the sweep rides both a publish and the poll, at most once an hour, so a machine that only receives still reaps. An envelope is dropped only when both its ULID mint time and its file's mtime clear the window: a rewritten mtime only delays the reap, and a long-offline sender's first publish never deletes itself. A daemon polls every `--poll-seconds` (15 by default) and tolerates a folder that is not mounted yet — but a publish creates the folder it was given, so a misspelled root is a local directory nobody else syncs, healthy in every check and read by nobody. Check the first envelope lands where you meant. `a8s health` does not tolerate a missing folder, and reports the one it could not reach or write. What this wire promises is at-least-once delivery — the consumed-ULID ledger and the inbox claim collapse the repeats — and nothing about order: two machines publishing into one folder arrive interleaved, because a sync client materializes files on its own schedule.
...
├── transactions.sqlite3      routing breadcrumbs for `a8s tx` / `a8s trace` (retains txlog_max_rows), and the remote address book `a8s ls` reads
...
Both SQLite stores are WAL, and a running node holds one connection to each so their `-wal` / `-shm` side files stay present: a reader that can read the a8s home but not write it — a sandboxed seat running `a8s convo` or `a8s tx` — can open a WAL store only while somebody holds it, and is told so by name when nothing does.
...
│   ├── folder.py     folder transport (a directory somebody else syncs)
│   └── s3.py         S3 transport (a bucket, for a machine that only has 443)
...
└── tests/
    ...
    ├── fixtures/     mock_cli.py + mock.json for end-to-end tests
...
Tests are isolated via a `fake_home` fixture that monkey-patches `HOME` to a tmp dir, so they never touch the real a8s state root. The daemon tests run real subprocesses against `tests/fixtures/mock_cli.py` (a deterministic script that echoes its argv, run through the interpreter the definition names as `$PYTHON`) so wake_once's argv expansion and routing fan-out can be asserted on the per-agent log.
...
A receipt made while the MQTT link is down waits on the receiver, up to 256
of them, and goes out in order on the next connection; its `RECEIPT_PUBLISHED`
row says `held until the link is back`. A node that stops sends every receipt
for mail it already took before it disconnects, and leaves mail that arrives
during the stop unacknowledged, so the broker offers it again. A full list,
or a stop with the link still down, drops receipts and logs how many.

A transport acknowledges only what the receive path reports consumed. Mail
the node could not take yet — an unreadable registry, a sibling daemon holding
the claim, a failed inbox write — stays unacknowledged: MQTT offers it again
after 1 second, doubling to 30, while the node runs, and the broker redelivers
whatever is left on the next session. The s3 and folder remotes leave the
object or file unrecorded and meet it again on the next poll.
