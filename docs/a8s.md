...
| `a8s convo <name> [--limit N] [-f] [--from NAME] [--glow [theme]]`                        | Markdown history of messages to or from an agent. Default `--limit 10`; this controls display only. `-f` follows sequence-numbered rows in `conversations.sqlite3` (shows outbound too — use `tells -f` for filedrop inbound-only). `--from NAME` (repeatable, case-insensitive) keeps only that sender's messages, in the backlog and under `-f`; the limit counts matches, not rows scanned. `a8s update` retains `convo_max_rows` rows (default 50000). Exit 0 with nothing printed means no messages; a store that is missing, unreadable, or not a conversation store at all is named on stderr with exit 1, and reading opens read-only — it never creates or initializes the file. `a8s transactions` and `a8s trace` draw the same line over `transactions.sqlite3`. |
