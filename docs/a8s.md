...
| `a8s ls [-q]`                  | List every name this node can reach. Registered nodes first, running or not: NAME, STATUS (`running (pid N)` / `stopped` / `unresolved: $KEY`), DEFINITION, ROOT, and bound namespaces. Then names only ever heard over a remote, with DEFINITION `remote` and the local time each last reached you. `-q` prints just names, both kinds. |
...
| `a8s remote <name> s3://<bucket>/<prefix> [--<opt> <value> ...]` | Register or overwrite an **s3** remote — envelopes travel through a bucket, which is the answer for a machine that can reach HTTPS on 443 and nothing else. One place, like the folder form; the scheme picks between them. The prefix is required and must be one nothing else in the bucket writes under, because this transport deletes what it delivers — `a8s remote` and `a8s storage` each refuse a prefix the other already owns. `--poll-seconds` (10, floored at 1), `--retain-days` (3 days; `0` keeps forever), `--region`, `--profile`, `--endpoint-url`, `--timeout-s` (60). Registration installs `boto3` through the `a8s-s3` group; credentials come from the standard chain (env, shared config, SSO, instance and container roles) and a8s never reads, stores or logs one. Attachments need their own service under a different prefix — `a8s storage <name> s3://<bucket>/a8s-files`. |
| `a8s unremote <name>`                                          | Forget a remote. Running daemons keep using the prior config until restart. A folder remote's consumed-envelope ledger goes with it, and so does the `sync_folder` service `a8s remote` created for that folder — identified by the `paired` marker that registration wrote into it, so a same-name service you configured yourself stays put even when it points at the same folder.                                                                                                                                                                                                                                                                                                    |


Remotes are git-shaped: an explicit list of places to fan messages out to. a8s only crosses cluster boundaries on `tell` / `prompt` — everything else (`a8s logs`, `a8s ls`, `a8s ps`) reads local state and nothing else. `a8s ls` naming remote nodes is not an exception: it reports who has reached *you*, read out of this machine's own transaction log, and asks nothing of the far side. A name last heard from weeks ago still lists, because the log records arrival, not liveness — but only while the log still holds a row for it: retention is `txlog_max_rows`, and a remote pruned out of it drops off the list until it speaks again. If you want cross-cluster log access, register an a8s connector that turns inbound tells into local `a8s logs` calls; a8s itself just enables the message + invocation path.

Each handler claims its own broker session: the MQTT client id is a hash of the machine, the handler's attached agent set, and the remote name, so several nodes share a host without evicting each other, and each one's queued messages survive a restart. `A8S_CLIENT_TAG` replaces the agent-set part, and a `client_id` option on the remote replaces the whole id. `a8s health` connects with a throwaway id and a clean session so a connectivity check never touches a node's queue.
...
A mailbox has exactly one owner, and the owner is a node. The node whose registry holds a name polls `<prefix>/<name>/` — every agent, alias and namespace prefix it holds, re-read on every poll so `a8s add` needs no restart — and it is the only node that may, because **deletion is the acknowledgement**: an envelope is deleted once the receive path reports it consumed, or it replays forever. Two nodes on one prefix would race to delete each other's mail. A node is not a process, though: every daemon on the machine runs its own subscriber, so several of them poll the same mailbox and are handed the same envelope. Whichever one wins `claim_message` consumes it and deletes the object; the others are told they did not, and leave it exactly where it is. The same rule is why a publish addressed to a name this node already answers for writes nothing: the routing pass delivers it locally, and the bucket would only hand the node its own message back. A name registered on two nodes is a foot-gun on any transport and is unworkable on this one. Delivery is at-least-once — a node that dies between the delivery and the delete sees the message again — and `claim_message` plus the seen-ids ring collapse the repeat exactly as they do for the other wires. There is no join cutoff here and none is wanted: a folder keeps every envelope every machine ever sent, while a mailbox is emptied by the one node that reads it, so whatever is waiting when that node comes back is mail addressed to it while it was away.

Retention is a bucket lifecycle rule, which survives a node that never comes back; nothing in a8s does. `--retain-days` covers only what the rule cannot see — an object under this node's own prefix that is not this transport's mail, because the key is not an envelope or the envelope is not the one its name claims. A well-formed envelope addressed to this node is never swept, however long it has failed to land: it is mail, and a node that cannot take it today may take it tomorrow. What is swept goes only when both the ULID mint time and the object's `LastModified` clear the window, the same two clocks the folder sweep requires, so a delayed send never sweeps itself; a key carrying no ULID has only the object clock to offer. A poll reads one page of at most 1000 keys per mailbox, and since it deletes what it consumes, a backlog drains over successive polls rather than turning one interval into an unbounded run of GETs on a network that is slow by assumption. `a8s health` exercises PUT, GET, LIST and DELETE against the configured prefix and removes its own probe — an IAM policy that grants three of the four is invisible until the missing verb is the one a message needs.
