...
The hazard is not the network, it is a half-arrived file. A sync client publishes a name before the bytes behind it are complete, and OneDrive's Files On-Demand shows a placeholder that only materializes when something reads it. So a file is written under a `.part` name and renamed, and a `manifest.json` records the size the receiver must see before it will accept the copy. A file that does not match yet simply is not there yet — the receiver's normal wait keeps looking. `--retain-days` sweeps old bundles; it is off by default, because deleting from a shared folder deletes from every machine sharing it.
